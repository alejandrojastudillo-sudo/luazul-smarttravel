...
console.log('app.js iniciado'); // 🔍 Para verificar carga
...
async function initSimulador() {
    console.log('initSimulador ejecutada'); // 🔍
    const selectGroup = document.getElementById('selectGroup');
    if (!selectGroup) {
        console.error('No se encontró #selectGroup');
        return;
    }
    try {
...
function construirSelects() {
    ...
    destinosHtml += '<option value="6">6 cuotas</option><option value="10" selected>10 cuotas</option>';
    destinosHtml += '<option value="12">12 cuotas</option><option value="18">18 cuotas</option>';
    destinosHtml += '</select>';
...
estilo.textContent = `
    ...
    .web-select option { background-color: #0c1628; color: #e8eeff; }
    .web-select:disabled { opacity: 0.6; cursor: not-allowed; }
    @media (min-width: 768px) {
        .select-group { display: flex; gap: 1rem; flex-wrap: wrap; }
        .web-select { flex: 1; margin-bottom: 0; }
    }
`;
